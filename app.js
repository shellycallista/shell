...
function openPostDetailModal(postId) {
    ...
    if (catBadge) {
        ...
        catBadge.className = post.category === 'Informatika'
            ? 'px-3 py-1 rounded-full text-xs font-bold bg-pink-100 text-pink-700'
...
function handleTTTClick(index) {
    ...
    if (checkTTTWin(tttCurrentPlayer)) {
        ...
        updateTTTScoreUI();
        setTTTStatusUI(`🎉 Pemain <span class="${tttCurrentPlayer === 'X' ? 'text-pink-600' : 'text-purple-600'} font-extrabold">${tttCurrentPlayer} Menang!</span>`);
    ...
    setTTTStatusUI(`Giliran: <span class="${tttCurrentPlayer === 'X' ? 'text-pink-600' : 'text-purple-600'} font-extrabold">Pemain ${tttCurrentPlayer}</span>`);
...
function updateTTTCellUI(index) {
    ...
    if (val === 'X') {
        cell.innerHTML = '<span class="text-pink-400 animate-scaleUp">X</span>';
    } else if (val === 'O') {
...
function updateTTTUI() {
    ...
    setTTTStatusUI(`Giliran: <span class="text-pink-600 font-extrabold">Pemain X</span>`);
}
